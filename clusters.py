...
class bicluster:
	...
	def drawnode(draw, x, y, scaling, labels):
		if clust.id < 0:
			...
			# If this is an endpoint, draw the item labels
	...
	def kcluster(rows, distance = pearson, k = 4):
		...
		ranges = [(min([row[i] for row in rows]), max([i] for row in rows))]
...
reload(clusters)
want, people, data=clusters.readfile('zebo.txt')
	...
	for m in range(0,1000):
		...
		for k in range(n):
			for j in range(n):
				...
				# point in proportion to how much error it has_key
...
def draw2d(data, labels, jpeg='mds2d.jpg'):
	img = Image.new('RBG',(2000,2000),(255,255,255))
	draw = ImageDraw.Draw(img)
